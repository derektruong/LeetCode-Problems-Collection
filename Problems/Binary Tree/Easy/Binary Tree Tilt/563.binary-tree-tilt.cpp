...
public:
    int findTilt(TreeNode* root) {
        ...
        return abs( sumLeft(root->left) - sumRight(root->right) ) + findTilt(root->left) + findTilt(root->right);
    }
    int sumLeft(TreeNode* left){
        if( !left ) return 0;

        return left->val + sumLeft(left->left) + sumLeft(left->right);
    }
    int sumRight(TreeNode* right){
        if( !right ) return 0;

        return right->val + sumRight(right->left) + sumRight(right->right);
    }
};
// @lc code=end
